...
当用户问“今天有什么 update”或要求总结“最新变化”时，默认比较区间是上一版报告的 issue time 到本版报告的 issue time。用户隔一天来，就总结这一天；隔一周来，就总结这一周。超过该区间的信息只能作为背景，不得写成新增信号。

规则：

- 先看信息发布时间，不能只看报告生成日期。
- 报告必须记录 `Report issued` 和 `Delta window`，后续人工总结以这个窗口为准。
- delta window 内没有新证据时，直接说“没有足够新增信号”，不要用窗口外材料填充。
- 公司财报、公告、业绩会 transcript、监管文件和主流新闻优先；转述类文章要降权。
- 结论变化必须来自新证据；旧结论只能标为“延续”，不能标为“新增”。
- 持仓跑路雷达同样只看 delta window 内的新触发器，除非旧事件在本窗口内有官方确认或新进展。

### 0.1 持仓记忆优先

每次检查持仓，必须先读 `docs/POSITION_MEMORY.md`，再读 `ai-radar/config.json` 里的 `holdings` 和 `holding_exit_radar`。

每个持仓都要按以下顺序彻底分析：

1. 原始买入逻辑和组合角色。
2. 最新 1 天窗口的新事件、价格、估值、市场信号。
3. 上次更新以来的新信息；旧事件的新转述只能作为背景，不能作为新增 delta。
4. 原始 thesis 是强化、未变、减弱，还是被证伪。
5. 对仓位动作的含义：加、拿、减、降为观察仓、退出，并写清触发条件。
6. 组合层面的作用：成长 beta、事件驱动期权、分红/防守对冲、质量底仓、流动性缓冲或风险抵消。

不要用泛主题新闻重新定义持仓：

- 美的集团 H 是分红、现金流和风险对冲底仓，不是默认 AI power/cooling 票。
- 快手-W 是可灵 / AI 视频商业化事件驱动仓，核心看融资、估值、收入和推理成本。
- 宁德时代是储能、电力基础设施和制造龙头重估，AI 数据中心用电/储能只是加分项。
...
在持仓映射之前，报告可以出现一个可选 section：

```text
## 持仓跑路雷达
```

这个 section 只在已有持仓命中负面触发器时显示；没有命中时不输出标题、不写“暂无”。它不是普通风险提示，只服务一个问题：是否需要跑路或降仓。

触发器只记录会破坏持仓逻辑的事件，例如：

- AI capex 掉头、云厂商下修数据中心或 AI 服务器订单。
- 光模块、AI PCB、DDR5/RCD/Retimer/CXL 等核心订单或毛利逻辑被证伪。
- 快手可灵分拆/融资失败，或可灵 ARR/收入显著低于预期。
- 宁德单 Wh 盈利、份额或储能利润率连续恶化。
- 腾讯游戏/广告失速，或 AI 投入拖累利润但商业化没有兑现。
- 美的扣非利润、现金流或海外业务连续恶化。
...
| 等级 | 含义 | 动作 |
|---|---|---|
| RED | 持仓核心逻辑被证伪 | 跑路或大幅降仓 |
| ORANGE | 赔率明显变差，但逻辑未完全死亡 | 降新增仓、降集中度 |
| YELLOW | 需要盯紧，暂不动作 | 观察，不追高 |
...
6. 如果出现 `持仓跑路雷达`，先处理触发器；没有出现则代表本轮没有命中强负面催化。
7. 最后看持仓映射，判断是否需要调整仓位、等待验证或补充研究。
