...
## Emergent Segment Discovery / 新瓶颈发现待办

这部分是我们投资框架本身的一部分：先从 AI factory 架构和瓶颈迁移出发，找未来 6-18 个月可能从边缘变量变成新约束的 segment，再用 KOL、研报、产业访谈、公司公告和股价拥挤度交叉验证。外部 read 的价值，是帮助我们发现市场已经在验证哪些瓶颈，而不是替代我们的判断。

当前不能直接照抄已经涨出来的海外票，也不能把已经被 A 股主线资金反复交易的映射标的当成 early idea。新 segment 只有同时满足“逻辑能从架构变化推导出来、公司级收入有可能验证、代表票尚未明显拥挤”时，才进入低拥挤观察篮。

| 处理 | 方向 / 标的 | 原因 | 下一步 |
|---|---|---|---|
| 剔除已拥挤 | 光模块/CPO、AI PCB/CCL/电子布、存储模组、铜连接显性票、液冷整机、AI 电源显性票、已启动测试设备票 | 这些方向仍是主线，但多数已经被市场充分命名；后续只做业绩上修和回撤机会复核，不再算低拥挤机会。 | 不纳入 early watchlist；只在估值、订单或盈利预测发生明显变化时复查。 |
| 保留观察 | 数据中心电力保护 / 低压配电 | AI 数据中心从买服务器走向重构电力链，熔断器、断路器、低压电器、配电保护可能是更隐蔽的消耗环节。 | 复核良信股份、中熔电气、宏发股份的 data-center/HVDC 订单、客户、收入占比和股价拥挤度。 |
| 保留观察 | 服务器电源上游 / 磁性器件 | 800V HVDC、48V、rack-scale power 架构升级可能提升变压器、电感、磁性元件价值量；比整机电源更上游。 | 优先复核可立克，比较麦格米特、欧陆通、新雷能是否已经过热。 |
| 保留观察 | 热管理材料 / 密封 / 导热 | 液冷整机已热，但 TIM、导热垫、密封胶、EMI、冷板材料等耗材可能仍处于更早阶段。 | 复核回天新材、硅宝科技、飞荣达、中石科技的 AI 服务器收入证据；没有收入拆分前不升级。 |
| 保留观察 | DDR5 配套 / SPD / EEPROM | memory wall 和服务器 DDR5 渗透会带来内存模组配套需求，但它不是澜起式强主线。 | 复核聚辰股份、国科微的服务器/DDR5/AI 相关收入和估值位置。 |
| 暂不升级 | 更远的泛控制器、泛流体部件、泛半导体设备 | 逻辑可能成立，但和 AI capex 的收入映射还弱。 | 只有出现客户、订单、产能或收入拆分证据才进入主观察池。 |

**当前候选池只作为待办，不作为推荐：** 良信股份、可立克、回天新材、硅宝科技、聚辰股份、国科微；中熔电气、宏发股份、飞荣达、中石科技列入灰名单，先检查拥挤度和真实 AI 收入暴露。
